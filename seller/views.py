...
@login_required         # 로그인이 필요한 뷰에는 로그인이 필요하다는 화면을 띄어준다.
def seller_index(request):
  foods = Food.objects.all().filter(user=request.user.id)
  context = {
    'object_list' : foods
  }
...
@login_required
def food_detail(request, pk):   # detail 화면은 pk를 받아야하고, object(하나의 데이터 덩어리, 제품 한개)를 전달
...
def food_delete(request, pk):
  object = Food.objects.get(pk=pk)
  object.delete()
  return redirect('seller/seller_index.html')
